...
(
	...
	VEN_STATUS	INT NOT NULL, --1 EN PROCESO, 2 TERMINADA, 3 CANCELADA
...
CREATE PROCEDURE TSP_nuevaVenta
(
	@CLIENTE INT
)
AS
BEGIN
	IF EXISTS(SELECT * FROM USUARIO WHERE @CLIENTE=USU_ID)
	BEGIN
		INSERT INTO TVENTA VALUES(@CLIENTE, 0,0,0,1,GETDATE())
		SELECT TOP 1 VEN_CVE_VENTA [VENTA] FROM TVENTA ORDER BY VEN_CVE_VENTA DESC
	END
	ELSE SELECT '0' [VENTA]
END
GO

CREATE PROCEDURE TSP_agregarCarrito
(
	@VENTA INT,
	@PRODUCTO INT,
	@CANTIDAD INT
...
AS
BEGIN
	IF EXISTS(SELECT * FROM TVENTA A, PRODUCTO B WHERE @VENTA=VEN_CVE_VENTA AND @PRODUCTO=PRO_CODIGO)
	BEGIN
		DECLARE @EXISTENCIA INT
		DECLARE @PRECIO FLOAT

		SELECT @EXISTENCIA=PRO_UNIDADES, @PRECIO=PRO_COSTO FROM PRODUCTO WHERE @PRODUCTO=PRO_CODIGO

		IF(@EXISTENCIA>=@CANTIDAD)
		BEGIN
			UPDATE PRODUCTO
			SET PRO_UNIDADES = @EXISTENCIA-@CANTIDAD 
			WHERE @PRODUCTO=PRO_CODIGO

			INSERT INTO TDETALLE_vENTA VALUES(@VENTA, @PRODUCTO, @CANTIDAD, @CANTIDAD*@PRECIO)

			SELECT '0'

		END
		ELSE SELECT '2' --EXISTENCIAS INSUFICIENTES
	END
	ELSE SELECT '1' --NO EXISTE EL PRODUCTO O LA VENTA
END
GO

CREATE PROCEDURE TSP_mostrarCarrito
(
	@VENTA INT
)
AS
BEGIN
	IF EXISTS(SELECT * FROM TVENTA WHERE VEN_CVE_VENTA=@VENTA)
	BEGIN
		SELECT PRO_NOMBRE [PRODUCTO], DET_CANTIDAD [CANT], DET_TOT [TOTAL] FROM TDETALLE_vENTA A, PRODUCTO B WHERE DET_CVE_VENTA=@VENTA AND PRO_CODIGO=DET_CVE_PRODUCTO
	END
	ELSE SELECT '1' -- NO EXISTE LA VENTA
END
GO

EXEC TSP_mostrarCarrito 1
SELECT * FROM TDETALLE_vENTA
